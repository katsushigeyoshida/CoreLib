...
namespace CoreLib
{
    ...
    public class DirectoryDiff
    {
        ...
        /// 比較元と比較先のファイルリストの登録
        /// </summary>
        /// <param name="srcDir">比較元ディレクトリ</param>
        /// <param name="destDir">比較先ディレクトリ</param>
        /// <param name="hashChk">ハッシュチェック</param>
        public DirectoryDiff(string srcDir, string destDir, bool hashChk = true)
        {
            ...
            foreach (FileInfo srcFile in mSrcFiles) {
                ...
                if (mHashChk)
                    mFiles[^1].mSrcCrc = mHash.GetCrc32L(srcFile.FullName);
            }
            foreach (FileInfo destFile in mDestFiles) {
                ...
                if (0 <= n) {
                    ...
                    if (mHashChk)
                        mFiles[^1].mDstCrc = mHash.GetCrc32L(destFile.FullName);
                }
            }
        }
